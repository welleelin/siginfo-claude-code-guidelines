...
```bash
# 手动检查更新
~/.claude/skills/check-updates/check-updates.sh
```

### 配置说明
在 `~/.claude/skills/check-updates/config.json` 中：
- `askOnStartup`: true - 启动时询问（默认）
- `skipForever`: false - 是否永久跳过询问
- 恢复询问：将 `skipForever` 设为 `false`
