...
│         │                                                   │
│         ▼                                                   │
│  Step 3: 执行 compact 命令压缩上下文                         │
│         │                                                   │
│         ▼                                                   │
│  Step 4: 读取上一个 checkpoint 的上下文                      │
│  ┌───────────────────────────────────────────────────────┐ │
│  │ - 从 MEMORY.md 读取检查点信息                           │ │
│  │ - 恢复任务执行状态                                     │ │
│  │ - 继续未完成的步骤                                     │ │
...
```json
// checkpoints/checkpoint-{timestamp}.json
{
  "checkpointId": "checkpoint-20260312-143000",
  "taskId": "task-52",
  "taskTitle": "实现用户登录功能",
  "phase": "implementation",
  "currentStep": "编写登录页面组件",
  "completedSteps": [
    { "step": "需求分析", "completedAt": "2026-03-12T14:00:00Z" },
    { "step": "任务规划", "completedAt": "2026-03-12T14:10:00Z" },
    { "step": "编写测试", "completedAt": "2026-03-12T14:20:00Z" }
  ],
  "variables": {
...
□ 1. 读取 MEMORY.md 持久化区域
   - 确认检查点信息（checkpointId、taskId、currentStep）
   - 确认测试状态
   - 确认 Mock 模式使用状态

□ 2. 恢复上一个 checkpoint 的上下文
   - 读取 checkpoints/checkpoint-{timestamp}.json
   - 加载任务执行状态
   - 恢复关键变量和决策
...
| 日期 | 版本 | 更新内容 | 更新人 |
|------|------|---------|--------|
| 2026-03-12 | 2.0.0 | 五阶段测试体系 + 安全渗透测试集成 | - |
